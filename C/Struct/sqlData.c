...
typedef struct SQLTable {
    ...
    void (*printData[]) (Data*); 
    // an array containing function pointers pointing to a function
} SQLTable;
...
void SQLquery(SQLTable *self, char cmd[]) {
    ...
    while (token != NULL) {
        if (!strcmp(token, "where")) {
            ...
            Data rhs;
            if (self->coltype[lhs] == INT)
                sscanf(token, "%d", &rhs.d.num); // read token and input to rhs.d.num
...
int main() {
    ...
    // transfer different fields into indice 
    ...
    scanf("%d", &Q);
    while (getchar() != '\n'); // clear every thing until next line
 
    char cmd[1024];
    for (int i = 0; i < Q; i++) {
        fgets(cmd, 1024, stdin); // fgets would read in '\n'
