...
int fsize(FILE *fp) {
    int prev = ftell(fp); // return the current reading position (start from 0)
...
int main() {
	...
	int size_n = fread(buf, 1, finsize, fin); // read data into buffer
...
// fseek(FILE *fp, long int offset, SEEK_CUR);