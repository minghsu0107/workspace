...
std::pair<int, std::pair<int, int>> minimax_optimization(char board[3][3], char marker, int depth, int alpha, int beta)
{
	...
	for (int i = 0; i < legal_moves.size(); i++)
	{
		...
		for (int q = 0; q < 3; q++)
			for (int k = 0; k < 3; k++)
				cur_board[q][k] = board[q][k];
		cur_board[curr_move.first][curr_move.second] = marker;

		// Maximizing player's turn
		if (marker == AI_MARKER)
		{
			int score = minimax_optimization(cur_board, PLAYER_MARKER, depth + 1, alpha, beta).first;

			// Get the best scoring move
			if (best_score < score)
			{
				// solution of deeper depth is not preferred
		...
		{
			int score = minimax_optimization(cur_board, AI_MARKER, depth + 1, alpha, beta).first;
...
int main()
{
	...
	while (!game_is_done(board))
	{
		...
		// find best solution dynamically
		// if human choose not the best decision, then search for the
		// best solution according to the condition now
