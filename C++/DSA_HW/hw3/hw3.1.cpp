...
private:
    ...
    {
        return static_cast<size_t>(std::ceil(num_elements / std::min(0.7, static_cast<double>(_max_load_factor))));
    }
...
uint64_t cardA[maxn];
uint64_t cardB[maxn];
ska::flat_hash_map<uint64_t, int> trans_table;
...
uint64_t set_board(uint64_t board, int i, bool maxplayer, long long alpha, long long beta) {
    ...
    board += (beta<<((n<<1) + 2));
    board += (alpha<<((n<<1) + 12));
    board &= ((1LL<<((n<<1) + 22)) - 1LL);
    if (maxplayer) {
        board ^= (1LL<<i);
        board += (cardA[i]<<((n<<1) + 22));
    }
    else {
        board ^= (1LL<<(i+n));
        board += (cardB[i]<<((n<<1) + 22));
    }
    return board;
}
int minimax(uint64_t board, bool maxplayer, long long alpha, long long beta) {
    ...
    if ((board & ((1LL<<n) - 1LL)) == 0 
        ...
        return get_remaining_points(board, maxplayer);
    }
    ...
    if (!flag) {
        ...
        cur_board += (alpha<<((n<<1) + 12));
        cur_board &= ((1LL<<((n<<1) + 22)) - 1LL);
        cur_board += (52LL<<((n<<1) + 22));
        if (maxplayer) {
            int score = minimax(cur_board, false, alpha, beta);
            best_score = max(best_score, score);
        }
    ...
    trans_table.insert(make_pair(board, best_score));
    return best_score;
}
