/*
給定一個字元陣列，欲按照Complete Binary Tree之位置規則建立一棵Binary Tree，
若陣列元素之資料為大寫字母(ASCII：65~90)，則將其建立成Tree的node，
若陣列元素為 ' x ' 則表示該位置沒有node
*/
#include <iostream>
#include <sstream>
#include <queue>
...
void BinaryTree::LevelorderConstruct(std::stringstream &ss){
    ...
    std::queue<TreeNode*> q;         // create a queue to handle level-order rule
    TreeNode *current = root;        // point *current to root
    char data = 'x';                 // initializa data as 'x'
    // read two character in each loop
    while (ss >> data) {
